...
class PyffleMta:
	...
	def sendMessage(self,instance):
		...
		fromhdr = ""
		if sender == None:
			## we're being called from a script of something
			fromhdr = "<%s>" % (fromaddress)
		else:
		...
		msg['X-Pyffle-MTA-Version'] = "%s" % (getIdentity())
		msg['X-Pyffle-Version'] = "%s" % (self.data.getPyffleVersionString())
		 

		with warnings.catch_warnings(record=True) as warn:
			## FIXME CRITICAL: need to sanitize the To header here...Shell injection
			print "TOHDR=" + tohdr
			sendmail = [self.data.static.options["pyffle.sendmail"], "-f %s" % (str(fromaddress)),tohdr]
