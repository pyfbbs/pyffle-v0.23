...
class PyffleData:
	...
	#####VERSION STRINGS####    - fixme, should be pulled from git or something	
	def getPyffleVersionString(self):
		return "Pyffle v0.23, freshly baked."
...
###
	...
	## Log a loginfailure
	def logLoginFail(self,username,reason="Bad credentials."):
		self.logEntry(self.LOGWARN,"LOGIN/FAILURE",str(username),"%s failed to log in, %s" % (str(username),str(reason)))


	def setCurrentlyonActivity(self, activity):
		entries = self.getSession().query(Currentlyon).filter(Currentlyon.id == self.sessionRef)
		entry = entries.first()
	...
	def purgeLogentries(self, maxEntries):
		entries = self.getSession().query(Logentry).order_by(desc(Logentry.date))
		count = 0 
...
#### Group membership functions
	...
	def addGroupMember(self,groupname,member):
		rv = False
		group = self.getGroup(groupname)
		if not group == None:
			if not self.getUser(member) == None:
				membership = Groupmember()
				membership.groupname = group.groupname
				membership.username = member
				self.session.add(membership)
				self.session.commit()
				rv = True
		return rv
	
	def getGroupMemberships(self,member):
		rv = []
		for theMembership in self.getSession().query(Groupmember).filter(Groupmember.username == member):
			rv.append(theMembership.groupname)
		return rv
	def getGroupMembership(self,group,member):
		rv = None
		for theMembership in self.getSession().query(Groupmember).filter(Groupmember.groupname == group.groupname).filter(Groupmember.username == member):
			rv = theMembership
			break
		return rv
	...
	def getGroupMembers(self,groupname):
		rv = []
		for theMembership in self.getSession().query(Groupmember).filter(Groupmember.groupname == groupname):
			rv.append(theMembership.username)
		return rv
	def isGroupMember(self,groupname,member):
		rv = False
	...
	def dropGroupMember(self,groupname,member):
		rv = False
		group = self.getGroup(groupname)
		if not group == None:
			if self.isGroupMember(group,member):
				membership = self.getGroupMembership(group,member)
...
#### Group functions
	...
	def deleteGroupByGroupname(self,name):
		## Get the board
		group = self.getGroup(name)
		if not group == None:
			## remove any members
			members = self.getGroupMembers(name)
			for member in members:
				self.dropGroupMember(name,member)
			## remove ourselves
			## but first lets remember our acl id so we can kill it after
			aclid = group.aclid
			self.getSession().delete(group)
			self.getSession().commit()		
			self.deleteAcl(aclid)
			
	
	
	def createGroup(self,name,description,aclid = None):
		#### creates a group
		myGroup = Usergroup()
		...
		if aclid == None:
			...
			myAcl.description="GROUP: "+name
			self.getSession().add(myAcl)
			self.getSession().commit()					
			aclid = myAcl.id
		myGroup.aclid = aclid
		self.getSession().add(myGroup)
		self.getSession().commit()
		self.logEntry(self.LOGNORMAL,"CREATE/GROUP","GROUPNAME:" + str(myGroup.groupname),"Group created: %s " % (str(myGroup.groupname)))
	...
	def getGroups(self):
		rv = []
		for theGroup in self.getSession().query(Usergroup).filter(Usergroup.groupname != "MONKEYBUSINESS!!"):
			rv.append( theGroup)
		return rv
	def getGroup(self, name):
		for theGroup in self.getSession().query(Usergroup).filter(Usergroup.groupname == name):
			return theGroup
		return None
...
### Board functions start
	...
	def getMessageIdsOlderThan(self, date, boardname = "__pyffle_email"):
		board = self.getBoardByName(boardname)
		boardid = board.id
		rv = []
		for theMsg in self.getSession().query(Message).filter(Message.boardid == boardid).filter(Message.sentdate <= date):
			rv.append(theMsg.id)
		return rv
	
	def getOldMessagesOver(self, boardname, maxmsgs):
		print "Got Boardname: " + boardname
		msgs = self.getMessagesByBoardname(boardname,checkSrm=False)
		count = 0
		rv = []
		for id in msgs:
			count = count + 1
...
### ACL functions start
	...
	def getGroupAces(self,username,grantordeny):
		memberships = self.getGroupMemberships(username)
		rv = []
		for membership in memberships:
			group = self.getGroup(membership)
			aclid = group.aclid
			aces =	self.getSession().query(Ace).filter(Ace.aclid==aclid).filter(Ace.grantordeny == grantordeny)
			if not aces == None:
				for myAce in aces:
					rv.append([myAce.permission,username])
		return rv
...
### ACE functions start
	...
	def isDenied(self,acl, subject, object, inherited=True):
		## we check the given ACL as well as any group ACLs if inherited is set to true
		denied = False
		myAces = self.getAces(acl.id,"DENY")
		if inherited:
			groupAces = self.getGroupAces(subject,"DENY")
			for groupAce in groupAces:
				myAces.append(groupAce)
		for myAce in myAces:
			if myAce[0] == object and myAce[1] == subject:
				denied = True
				break
		return denied
	...
	def isGranted(self,acl, subject, object, inherited=True):
		## 
		granted = False
		myAces = self.getAces(acl.id,"GRANT")
		if inherited:
			groupAces = self.getGroupAces(subject,"GRANT")
			for groupAce in groupAces:
				myAces.append(groupAce)
...
### MessageText handling
	...
	def searchMessagetexts(self,text,boardname=None,searchSpecial = False):
		rv = []
		query = self.getSession().query(Messagetext).filter(Messagetext.msgtext.like('%' + text + '%'))
		for msgtext in query:
			msg = self.getMessage(msgtext.messageid)
		
			boardOK = False
			if not boardname == None:
				board = self.getBoardByName(boardname)
				if (msg.boardid == board.id):
					boardOK = True
			else:
				board = self.getBoard(msg.boardid)
				if not board == None:
					if not searchSpecial:
...
### Message functions
	...
	def getBoardMessageCount(self,boardname):
		theboard = self.getBoardByName(boardname)
		rv = self.getSession().query(Message).filter(Message.boardid == theboard.id).count()
		return rv
	...
	def searchMessageSubjects(self,text,boardname=None,searchSpecial=False):
		rv = []
		query = self.getSession().query(Message).filter(Message.subject.like('%' + text + '%'))
		for msg in query:
			boardOK = False
			if not boardname == None:
				board = self.getBoardByName(boardname)
	...
	def _dumpMessages(self):
		...
		if True: ##self.util.DEBUG:
			for instance in self.getSession().query(Message).order_by(Message.id): 
				...
				self.util.debugln("From: " + str(instance.fromname).strip())
				self.util.debugln("To: " + str(instance.toname).strip())
				self.util.debugln("Subj: " + str(instance.subject))
				self.util.debugln("--------------------------------------------------------------")
	...
	def createMessage(self,fromname,toname,subject,text,board='__pyffle_email'):
		...
		if board == '__pyffle_email':
			...
			## So we grab the board's acl and use it for the message
			## FIXME - make a COPY of the board's ACL
			msgAcl = self.createAcl()
