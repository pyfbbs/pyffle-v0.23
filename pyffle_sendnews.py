...
##			fromhdr = "%s <%s>" % (sender.fullidentity,fromaddress)
		...
		for hostname in feeds:		
			...
			with warnings.catch_warnings(record=True) as warn:
				...
				if self.doNntp:
					cmdline = "/pyffle/nntpout.py " + hostname
...
##				mailout.close()
##				mailerr.close()
#				print "rnews status=" + status
#				print "rnews error=" + error
	...
	def go(self,nntp):	
		...
		for boardextname in boards:
			...
			for msgid in msgids:
				...
				self.data.logEntry(self.data.LOGINFO,"EXPORT/MESSAGE/NEWS","sendnews","sendnews POSTED msgid " + str(msgid))
